...
> - the adoption note `.claude/togi.md`
...
After completing all steps, print a summary of what was added vs. already present for each file touched: `.claude/togi.md`, the `CONTRIBUTING.md`/`README.md` pointer, and `.gitignore`.
...
- **This repo only** — run `/togi:enable repo`.
- **All my repos** — run `/togi:enable all`.
...
1. Pick the base remote: `upstream` if it exists (fork workflow — `origin` is the fork and may be stale), otherwise `origin`. Call it `<remote>` below. Detect that remote's default branch, falling back to `main` if it can't be determined.
...
4. Stage exactly the three Phase 2 files — `.claude/togi.md`, `.gitignore`, and the pointer file — then commit with message: `chore: set up togi`
5. Push the branch to `origin` (on a fork that is your fork; `gh pr create` then targets the upstream repo automatically), and open a PR titled `Set up togi` with this body (adjust the pointer filename; append the standard `Generated with Claude Code` footer):
   ...
   **Is this safe to merge?** Yes — and you can verify it from the diff alone. The entire change is three inert text files: the adoption note `.claude/togi.md`, a pointer in `CONTRIBUTING.md`, and `.gitignore` entries. No settings, no hooks, no code: merging installs nothing and runs nothing on anyone's machine. If the diff shows anything beyond those three files, reject this PR.

   **Trying it costs a minute and pennies.** Run the three commands in `.claude/togi.md`, work normally, and check `.claude/friction/pending/` after a few sessions. Capture is opt-in per developer: a headless sweep — Claude resuming your ended session under your own credentials, nothing sent to any third party — reviews each session for friction events. Typical cost: $0.05–$0.20 per session, billed at standard API rates from your separate Agent SDK credit (or API billing). Haven't opted in? You'll see a one-time notice and nothing else will ever run. Leave any time with `/togi:disable`.
   ```
