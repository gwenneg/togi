...
The `SessionStart` hook delivers the directive fresh each session, but it sits near the top of context and gets buried as the session grows — and compaction can drop it entirely. The `Stop` hook (which fires every turn) counters both. It measures **context size** from the transcript — `input_tokens + cache_read_input_tokens + cache_creation_input_tokens` of the most recent assistant `usage` block, read from the tail so a large transcript never makes the hook slow — and compares it to the last refresh. On **growth** past a threshold it re-injects a short salience **nudge**; on a **sharp drop** (compaction just fired and may have dropped the directive) it re-injects the **full directive** — because [`PostCompact` cannot inject context](https://code.claude.com/docs/en/hooks) (verified), the `Stop`-detected drop is the only place to repair it. The first turn just records a baseline (`SessionStart` already delivered the directive). Both injections go through `hookSpecificOutput.additionalContext`, a silent system reminder the user never sees; `Stop` does not inject plain stdout, so the JSON field is required.
...
Nothing leaves your machine. Capture happens inside your own session under your own credentials; friction notes are written locally under `.togi/friction/` (`pending/`, then `archive/` once processed — both git-ignored), and nothing is sent to any third party. The capture directive (`assets/prompts/friction-capture.md`, shipped in the plugin) is plain, reviewable instructions, and denied tool calls are recorded by the `PermissionDenied` hook with no model in the loop. Any developer can opt out with `/togi:disable`.
...
- Context size is recoverable from the transcript: `input_tokens + cache_read_input_tokens + cache_creation_input_tokens` of the most recent assistant `usage` block is the input the model saw that turn (live-verified — one session's last turn read ~555K). The `Stop` hook reads this from the tail to detect growth and the post-compaction drop ([§1](#1-architecture--lifecycle)).
...
- `PostCompact` cannot inject context (no `additionalContext`, no decision control — "shows stderr only"), so the `Stop`-detected size drop is the only way to reinject the directive after compaction. `SessionStart` supports `additionalContext` plus a `systemMessage` in the same output ([§1](#1-architecture--lifecycle)) — [hooks reference](https://code.claude.com/docs/en/hooks).
