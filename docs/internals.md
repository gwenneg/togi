...
Each rejected alternative (1–8) has a standalone write-up under [`alternatives/`](alternatives/) — what it is, why it wasn't chosen, and a plan to implement it if revisited.

### Why the resume sweep was chosen

**9. Resume sweep** — full-session hindsight from the model that lived the session (verified: fork carries complete context, original transcript untouched), zero session noise, and the cheapest of the high-recall options because the warm prompt cache replays the session at ~0.1× input price. Trades away: one API call per session (consent + cost disclosure required), no sweep on crashed sessions, detached-process fragility.
