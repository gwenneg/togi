...
| `TOGI_NUDGE_INTERVAL` | `10` | Turns between salience nudges from the `Stop` hook. Lower if friction recall feels low in medium-length sessions; raise to reduce nudge frequency in very long ones. See [§1](#1-architecture--lifecycle) for the research rationale behind turn count as the signal. |
