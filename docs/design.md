...
- The friction file is an object: an optional per-sweep `sweep_cost_usd` header and an `events` array. Each field lives where the skill consumes it — the per-sweep cost aggregate in the header (summed across files for the PR), and everything consumed *per event* on the event itself: `date` (the recurrence comparison, display, and PR line all operate per event, and events get regrouped across sweeps, so the date travels with each), `body` (drives grouping, placement, the recurrence match, and the edit), `type` (PR metrics + display label), `captured_by` (the confidence signal — an event captured by the Haiku fallback is lower-confidence; the *model*, not the cache state, sets capture quality), and optional `misleading_doc` (a high-confidence placement hint). The session-start counter reads `.events | length`.
