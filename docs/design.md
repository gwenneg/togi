...
- Envelope fields used: `.result` (the events array, now a JSON-encoded string — extracted with `fromjson`, every failure degrading to zero events as before), `total_cost_usd` → `sweep_cost_usd`, `usage.cache_read_input_tokens` → `sweep_cache_read_tokens`, `usage.cache_creation_input_tokens` → `sweep_cache_creation_tokens` (cache writes are a suspected cost driver: 1h-TTL writes bill at 2× base input, and a prefix mismatch re-writes the whole session at that premium), `is_error`/`duration_ms`/fork `session_id` → debug log only. Ignored: `modelUsage` (redundant with `captured_by` + logged model args), `num_turns`, `iterations`. All telemetry fields are optional everywhere — the friction write must never fail for lack of them.
- The friction file is an object: session-level metadata (`session`, `date`, `cache`) and sweep telemetry once in the header, pure capture fields in `events`. First shipped (v0.5.1) as per-event stamps on a flat array; reverted the same day on review — session-level information does not belong on events. The session-start counter reads `.events | length`; `update-context-docs` attaches the header fields to events when flattening.
